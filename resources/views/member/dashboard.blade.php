    ...
    <div class="col-12">
        <div class="welcome-hero" style="background: linear-gradient(135deg, var(--primary-color), var(--primary-light)); color: white; padding: 3rem; border-radius: 20px; position: relative; overflow: hidden;">
            ...
            <div style="position: relative; z-index: 2;">
                ...
                <p class="lead mb-4">Ready to explore the world of knowledge?</p>
                ...
                @else
                    <div class="d-flex gap-3">
                        ...
                        <a href="{{ route('member.borrowings.index') }}" class="btn btn-outline-light btn-lg" style="border: 2px solid white;">
...
<div class="row g-4 mb-5">
    <div class="col-md-3">
        <div class="stats-card-member position-relative">
            ...
            <div class="small text-white-50 mb-1">Currently Borrowed</div>
            <h3>{{ auth()->user()->borrowings()->where('status', 'approved')->count() }}</h3>
            <a href="{{ route('member.borrowings.index') }}" class="btn btn-sm btn-light mt-2">
    ...
    <div class="col-md-3">
        <div class="stats-card-member success position-relative">
            ...
            <div class="small text-white-50 mb-1">Total Returned</div>
            <h3>{{ auth()->user()->borrowings()->where('status', 'returned')->count() }}</h3>
            <a href="{{ route('member.borrowings.history') }}" class="btn btn-sm btn-light mt-2">
    ...
    <div class="col-md-3">
        <div class="stats-card-member warning position-relative">
            ...
            <div class="small text-white-50 mb-1">Pending Approval</div>
            <h3>{{ auth()->user()->borrowings()->where('status', 'pending')->count() }}</h3>
            <div class="btn btn-sm btn-light mt-2 disabled">
                Waiting... <i class="fas fa-hourglass-half ms-1"></i>
            </div>
        </div>
    </div>
    ...
    <div class="col-md-3">
        <div class="stats-card-member danger position-relative">
            ...
            <div class="small text-white-50 mb-1">Overdue Books</div>
            ...
            <div class="small mt-2">
                @if(auth()->user()->borrowings()->whereDate('due_date', '<', now())->where('status', 'approved')->count() > 0)
                    <i class="fas fa-info-circle me-1"></i>Please return ASAP!
                @else
                    <i class="fas fa-check me-1"></i>Great! No overdue
                @endif
            </div>
        </div>
    </div>
...
<div class="row g-4">
        ...
        <div class="card-modern">
            <div class="card-header-modern">
                <div class="d-flex justify-content-between align-items-center">
                    <h5 class="mb-0"><i class="fas fa-bookmark me-2"></i>My Active Books</h5>
                    <a href="{{ route('member.borrowings.index') }}" class="btn btn-sm btn-light">View All</a>
                </div>
            </div>
            <div class="card-body p-0">
                ...
                @empty
                    <div class="p-5 text-center">
                        ...
                        <h5 class="text-muted mb-2">No Active Borrowings</h5>
                        <p class="text-muted">Start exploring our collection!</p>
    ...
    <div class="col-lg-4">
        ...
        <div class="card-modern">
            <div class="card-header-modern">
                <h6 class="mb-0"><i class="fas fa-chart-line me-2"></i>My Statistics</h6>
            </div>
            <div class="card-body">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if($activity->status == 'returned')
                                            <i class="fas fa-undo text-success me-1"></i>Returned
                                        @else
                                            <i class="fas fa-download text-primary me-1"></i>Borrowed
                                        @endif
                                    </td>
                                    ...
                                    <td>
                                        ...
                                        @elseif($activity->status == 'returned')
                                            <span class="badge badge-modern bg-info">Returned</span>
                                        @else
